...
from config import BACKEND_HOST, BACKEND_PORT, BYTEPLUS_API_KEY, CORS_ORIGINS, GEMINI_API_KEY, OUTPUTS_DIR
from model import (
    ASPECT_RATIOS,
    IMAGE_QUALITIES,
    MODEL_PRESETS,
...
app = FastAPI(title='Nano Banana Local Web', version='1.2.0')
...
def presets() -> PresetsResponse:
    return PresetsResponse(models=MODEL_PRESETS, aspect_ratios=ASPECT_RATIOS, qualities=IMAGE_QUALITIES)
...
async def generate(
    ...
    if use_reference_prompt and not GEMINI_API_KEY:
        raise HTTPException(status_code=500, detail='Pre Reference Prompt Mode potrebuješ GEMINI_API_KEY v backend/.env')
    ...
    for idx, upload in enumerate(uploads, start=1):
        ...
        references.append(ReferenceImage(slot=idx, name=upload.filename, mime_type=mime_type, data=file_bytes))
    ...
    for i in range(batch_count):
        try:
            ...
            results.append(GenerateResponseItem(index=i, ok=False, error=str(exc)))
...
def list_outputs() -> dict:
    files = sorted([p.name for p in OUTPUTS_DIR.glob('*.jpg')], reverse=True)
    return {'files': files}
